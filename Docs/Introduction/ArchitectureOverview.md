...
## レイヤ構造
このライブラリでは、下位層であるHAL（Hardware Abstraction Layer）を抽象化し、より扱いやすいインターフェースを提供するための中間層として設計されています。
上位のアプリケーションはハードウェアの細かい実装や制御仕様を気にすることなく機能にアクセスできます。

## HALラッパーの全体的なアーキテクチャ図や説明
```
[ Application ]
      │
[ HALラッパー ]
...
- HALの抽象化・簡略化  
  複雑なHAL APIをシンプルなインターフェースで提供し、開発者が容易に利用できるようにします。
- 初期化の簡素化  
